...
After invoking the deployed App at least once, verify the trace contract with `MLFLOW_TRACING_SQL_WAREHOUSE_ID="$DATABRICKS_SQL_WAREHOUSE_ID" uv run python -m evaluation.verify_traces --experiment "${MLFLOW_EXPERIMENT_NAME:-/Shared/globalmart-supply-chain-agent-uc-v2-dev}" --trace-location globalmart.agent_observability.contract_agent_traces`.
