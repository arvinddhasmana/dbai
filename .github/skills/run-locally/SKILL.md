...
Use `mlflow.genai.evaluate(...)` with an evaluation dataset and scorers so the
run appears in the Databricks Experiment Evaluation and Score views. Keep the
judge model separate from the agent model. When evaluating a deployed App,
send requests to its `/invocations` route; do not call the App root URL.
