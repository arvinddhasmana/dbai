...
Keep evaluation code in a separate evaluation bundle or module. Publish
evaluation rows with `mlflow.genai.evaluate(data=..., scorers=...)` so
Databricks Experiments show Evaluation and Score records. Ordinary MLflow
metrics and artifacts are supplemental only. For deployed Apps, evaluate via
`POST /invocations` and use a separate low-cost judge endpoint.
