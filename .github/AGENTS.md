...
For Databricks agent evaluations, keep evaluation code outside production agent
modules and use the MLflow GenAI Evaluation API:

```python
mlflow.genai.evaluate(data=rows, predict_fn=predict_fn, scorers=scorers)
```

This creates Experiment Evaluation and Score records. Ordinary
`mlflow.start_run()` metrics and artifacts may supplement the report, but they
do not replace `mlflow.genai.evaluate`. For a deployed App, invoke the
MLflow AgentServer `/invocations` route, keep the judge endpoint separate from
the agent endpoint, and log deterministic retrieval metrics plus judge scores
as evaluation scorers.
